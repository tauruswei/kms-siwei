...
public class TestGetWhiteCert {
    ...
    public static void test(KMS_SIWEI kms_siwei, String para_kmsuser, String para_kmspassword, String para_name, int threadNum, int count) {
        ...
        new Thread(new TPSPrint(GetWhiteCert.class, "GetWhiteCert")).start();
        //启动停止线程
        new Thread(new StopTest(GetWhiteCert.class, TPSPrint.class)).start();
        for (int i = 0; i < threadNum; i++) {
            new Thread(new GetWhiteCert(kms_siwei, para_kmsuser, para_kmspassword, para_name,  count)).start();
        }
...
//    }
     ...
     * @author
     */
    public static class GetWhiteCert implements Runnable {
        ...
        public GetWhiteCert(KMS_SIWEI kms_siwei, String para_kmsuser, String para_kmspassword, String para_name,  int count) {
            okCount = 0;
            failCount = 0;
            exCount = 0;
            running = true;
