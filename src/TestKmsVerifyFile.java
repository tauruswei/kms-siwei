...
public class TestKmsVerifyFile {

    public static void test(KMS_SIWEI kms_siwei, String para_cert_path,String para_algName,String para_plainPath,String para_signPath, int threadNum, int count) {

        new Thread(new TPSPrint(KmsVerifyFile.class, "KmsVerifyFile")).start();
        //启动停止线程
        new Thread(new StopTest(KmsVerifyFile.class, TPSPrint.class)).start();
        for (int i = 0; i < threadNum; i++) {
            new Thread(new KmsVerifyFile(kms_siwei, para_cert_path,para_algName, para_plainPath, para_signPath, count)).start();
        }
...
//    }
     ...
     * @author
     */
    public static class KmsVerifyFile implements Runnable {
        ...
        public KmsVerifyFile(KMS_SIWEI kms_siwei,String para_cert_path,String para_algName, String para_plainPath,String para_signPath, int count) {
            okCount = 0;
            failCount = 0;
            exCount = 0;
            running = true;
