...
public class TestGetCertKey {
    ...
    public static void test(KMS_SIWEI kms_siwei, String para_kmsuser, String para_kmspassword, String para_name, int threadNum, int count) {
        ...
        new Thread(new TPSPrint(GetCertKey.class, "GetCertKey")).start();
        //启动停止线程
        new Thread(new StopTest(GetCertKey.class, TPSPrint.class)).start();
        for (int i = 0; i < threadNum; i++) {
            new Thread(new GetCertKey(kms_siwei, para_kmsuser, para_kmspassword, para_name,  count)).start();
        }
...
//    }
     ...
     * @author
     */
    public static class GetCertKey implements Runnable {
        ...
        public GetCertKey(KMS_SIWEI kms_siwei, String para_kmsuser, String para_kmspassword, String para_name,  int count) {
            okCount = 0;
            failCount = 0;
            exCount = 0;
            running = true;
        ...
        public void run() {
            ...
            while ((running)) {
                try {
                    Result result = kms_siwei.getCertKey(para_kmsuser, para_kmspassword, para_name);
