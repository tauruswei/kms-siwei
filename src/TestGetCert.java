...
import com.sansec.asn1.cmc.GetCert;
...
public class TestGetCert {
    ...
    public static void test(KMS_SIWEI kms_siwei, String para_kmsuser, String para_kmspassword, String para_name, int threadNum, int count) {
        ...
        new Thread(new TPSPrint(GetCert.class, "GetCert")).start();
        //启动停止线程
        new Thread(new StopTest(GetCert.class, TPSPrint.class)).start();
        for (int i = 0; i < threadNum; i++) {
            new Thread(new GetCert(kms_siwei, para_kmsuser, para_kmspassword, para_name,  count)).start();
        }
...
//    }
     ...
     * @author
     */
    public static class GetCert implements Runnable {
        ...
        public GetCert(KMS_SIWEI kms_siwei, String para_kmsuser, String para_kmspassword, String para_name,  int count) {
            okCount = 0;
            failCount = 0;
            exCount = 0;
            running = true;
        ...
        public void run() {
            ...
            while ((running)) {
                try {
                    Result result = kms_siwei.getCertificate(para_kmsuser, para_kmspassword, para_name);
