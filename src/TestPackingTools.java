...
import com.sansec.engine.packtools.PackUtils;
...
public class TestPackingTools {

    public static void test(int threadNum, int count) {

        new Thread(new TPSPrint(PackingTools.class, "PackingTools")).start();
        //启动停止线程
        new Thread(new StopTest(PackingTools.class, TPSPrint.class)).start();
        for (int i = 0; i < threadNum; i++) {
            new Thread(new PackingTools(count)).start();
        }
...
//    }
     ...
     * @author
     */
    public static class PackingTools implements Runnable {
        ...
        public void run() {
            ...
            while ((running)) {
                try {
                    byte[] bytes = Util.readFromFile("E:\\worklist\\publicCertificate.txt");
                    String s = PackUtils.genEnvelopedData(new String(bytes), bytes);
